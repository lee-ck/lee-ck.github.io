...
여기서 $x_{N}^{*}=\mathbf{0}$ 이기 때문에 $x_{N+1}=A x_{N}^{*} + B\mathbf{0} =\mathbf{0}$ 을 만족. 따라서 $x_1$에서 계산한 solution도 zero terminal state constraints 를 만족. &rarr;  "***Recursive Feasiblity***"
