...
여기서 $x_{N}^* = \mathbf{0}$ 이기 때문에 $x_{N+1}=A x_{N}^{*} + B\mathbf{0} =\mathbf{0}$ 을 만족. 따라서 $x_{1}$에서 계산한 solution도 zero terminal state constraints 를 만족. &rarr;  "***Recursive Feasiblity***"
...
초기시간 $t$ 에서의 최적해 $U^{*}_{t}$와 $X^{*}_{t}$를 통해 계산되는 optimal cost = $J^*(t)$. 그 다음 시간 $t+1$ 에서의 해는 non-optimal 이기 때문에 $\hat{J}(t+1)$으로 표기.
...
이를 통해 $J^{*}(t+1) \leq \hat{J}(t+1)\leq J^{*}(t)$ 임을 알 수있고 따라서 Cost는 0으로 수렴한다는 것을 알 수 있음.
...
$x^{*}_{N} \in\mathbb{X}_f$ 이고 *Assumption 2*.에 의하여 $x_{N+1}\in \mathbb{X}_f$도 역시 만족함. &rarr;  "***Recursive Feasiblity***"
